 ...
 */
public class PlayerPlaceCommand extends NucleusPlayerCommand implements GroupCommand, StateSetCommand {
    public PlayerPlaceCommand() {
        super(0, 1);
    }

    @Override
    protected void subOnCommand(Player player, Command command, String s, String[] strings) throws Exception {
        if (PlayerStateManager.getInstance().get(player.getUniqueId()).getReinforcementState() == ReinforcementState.PLACE && strings.length == 0) {
            PlayerStateManager.getInstance().get(player.getUniqueId()).setReinforcementState(ReinforcementState.NONE);
            player.sendMessage(ChatColor.GREEN + "Reinforcement mode PLACE now " + ChatColor.RED + "off.");
        } else {
            Group groupID = null;
            if (strings.length == 0) {
            ...
            if (!material.isPresent()) {
                ...
                setState(player, material.get(), groupID, ReinforcementState.PLACE);
            }
        }
    }
