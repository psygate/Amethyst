 ...
 */
public class PlayerPunchCommand extends NucleusPlayerCommand implements GroupCommand, StateSetCommand {
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] strings) throws Exception {
        if (PlayerStateManager.getInstance().get(player.getUniqueId()).getReinforcementState() == ReinforcementState.PUNCH && strings.length == 0) {
            PlayerStateManager.getInstance().get(player.getUniqueId()).setReinforcementState(ReinforcementState.NONE);
            player.sendMessage(ChatColor.GREEN + "Reinforcement mode PUNCH now " + ChatColor.RED + "off.");
        } else {
            Group groupID = null;
            if (strings.length == 0) {
            ...
            if (!material.isPresent()) {
                ...
                setState(player, material.get(), groupID, ReinforcementState.PUNCH);
            }
        }
    }
