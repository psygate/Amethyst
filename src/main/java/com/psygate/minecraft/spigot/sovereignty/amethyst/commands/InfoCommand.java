...
import com.psygate.minecraft.spigot.sovereignty.amethyst.events.PlayerSetInfoModeEvent;
 ...
 */
public class InfoCommand extends NucleusPlayerCommand implements YesNoCommand {
    public InfoCommand() {
        super(0, 1);
    }
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] args) throws Exception {
        ...
        PlayerStateManager.getInstance().get(player.getUniqueId()).setInspecting(newMode);
        player.sendMessage("Info mode now " + ((newMode) ? ChatColor.GREEN : ChatColor.RED) + asString(newMode));
    }

    private boolean getModeToSet(UUID uuid, String[] args) {
        if (args.length == 0) {
            return !PlayerStateManager.getInstance().get(uuid).isInspecting();
        } else {
